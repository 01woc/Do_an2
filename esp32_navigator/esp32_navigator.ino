...
TinyGPSPlus gps;
HardwareSerial GPSSerial(2); // Sử dụng UART2 của ESP32
...
const unsigned long GPS_UPDATE_INTERVAL = 1000; // Cập nhật GPS mỗi giây
...
void updateGPS() {
  while (GPSSerial.available() > 0) {
    if (gps.encode(GPSSerial.read())) {
      if (gps.location.isValid()) {
        ...
        // Tạo chuỗi vị trí
        gpsLocation = String(gpsLatitude, 6) + ", " + String(gpsLongitude, 6);
  ...
  // Kiểm tra nếu GPS không hoạt động
  if (millis() > 5000 && gps.charsProcessed() < 10) {
    Serial.println("GPS not detected. Check wiring.");
  }
}

// Hàm hiển thị thông tin GPS lên màn hình
void displayGPSInfo() {
  if (!gpsValid) {
    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.setTextSize(1);
    tft.setCursor(10, 10);
    tft.println("GPS: No Signal");
    return;
  }
